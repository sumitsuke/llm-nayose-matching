...
def action(r,cond): d=r.get(cond); return d.get("action") if isinstance(d,dict) else None
...
def wilson(k,n,z=1.96):  # Wilson score 95% CI in percent (pre-registered for destruction/asymmetry)
    ...
    h=z*sqrt(p*(1-p)/n+z*z/(4*n*n))/den; return (100*(c-h),100*(c+h))
...
print(f"  discordant: Cのみ正解={c_not_b} / Bのみ正解={b_not_c} → exact McNemar 両側 p = {mcnemar_exact(b_not_c,c_not_b):.3f}")
print(f"  注: 主結果は件数(上の2x2)。McNemar/bootstrapCIは補助(discordant={b_not_c+c_not_b}で検出力実質ゼロ・CI下限も退化)。")
...
for name,rows in [("確認120",conf),("難例40",hard),("全160",res)]:
    print(f"  {name}: A正解{sum(correct(r,'A') for r in rows)} / B正解{sum(correct(r,'B') for r in rows)} / C正解{sum(correct(r,'C') for r in rows)}")
gb_bc=sum(1 for r in res if correct(r,"B") and not correct(r,"C"))
gc_bc=sum(1 for r in res if not correct(r,"B") and correct(r,"C"))
gdB=sum(1 for r in res if correct(r,"A") and not correct(r,"B")); gdC=sum(1 for r in res if correct(r,"A") and not correct(r,"C"))
print(f"  全160 B対C discordant: B正C誤{gb_bc} / B誤C正{gc_bc} → McNemar exact p={mcnemar_exact(gb_bc,gc_bc):.3f} {'(1対1で対称=完全同点)' if gb_bc==gc_bc else ''}")
print(f"  全160 破壊(A正解→誤): A→B {gdB}件 / A→C {gdC}件 {'(同数)' if gdB==gdC else ''} → 確認サブセットの非対称(B3/C2)は全体で相殺")
...
def transitions(rows,target):
    ...
    for r in rows:
        ...
        tt+=a and t; ft+=(not a) and t; tf+=a and (not t); ff+=(not a) and (not t)
    return tt,ft,tf,ff
print("\n--- 副次: 初回Aからの遷移 A→B と A→C を並記 (確認120) ---")
print(f"  {'遷移':<16} {'A→B':>5} {'A→C':>5}")
tB=transitions(conf,"B"); tC=transitions(conf,"C")
...
print(f"  → 破壊は自己レビュー固有でない: 確認セットでBが{tB[2]}件/Cが{tC[2]}件だが、これは確認サブセット限定。全160では破壊も同数(上記)。")
# 生件数+Wilson。破壊は実在(件数>0)、"ゼロかの検定"ではない。修正0との非対称は少数・順序固定ゆえ強調しない。
nAc=sum(correct(r,"A") for r in conf)  # denom = Aが正解だった件数(確認)
loB,hiB=wilson(tB[2],nAc); loC,hiC=wilson(tC[2],nAc)
print(f"  破壊率(A正解{nAc}件中・生件数+Wilson95%): A→B {tB[2]}/{nAc}={100*tB[2]/nAc:.2f}% [{loB:.2f},{hiB:.2f}] / A→C {tC[2]}/{nAc}={100*tC[2]/nAc:.2f}% [{loC:.2f},{hiC:.2f}]")
# C=A 一致率: Cは実質Aのコピー。食い違う分は全てC悪化。
ca_conf=sum(1 for r in conf if dec(r,"C")==dec(r,"A")); ca_all=sum(1 for r in res if dec(r,"C")==dec(r,"A"))
...
print(f"  うち実際に判定を覆した={flip} (そのうち破壊={flip_bad}) / 宣言CHANGEだが判定不変={declared_no_flip} (自己申告≠実変更)")
...
for c in "ABC":
    ...
    lf,hf=wilson(fm,60); lm,hm=wilson(mm,60)
    print(f"  {c}: 誤統合(false-merge)={fm}/60 [Wilson {lf:.1f},{hf:.1f}%]  見逃し(missed-match)={mm}/60 [Wilson {lm:.1f},{hm:.1f}%]  無効出力={ab}")
...
thB=transitions(hard,"B"); thC=transitions(hard,"C")
...
json.dump({"delta_C_minus_B":d_hat,"ci":[lo,hi],"BA":{c:ba(c) for c in "ABC"},
           "primary_CvsB_confirmation":{"both_correct":bc_bC,"B_only":b_not_c,"C_only":c_not_b,
                "neither":neither,"mcnemar_exact_p":mcnemar_exact(b_not_c,c_not_b)},
           ...
           "fixable_errors":{"n":len(a_wrong),"fixed_by_C":sum(1 for r in a_wrong if correct(r,"C")),
                "detail":[{"pair_id":r["pair_id"],"kind":r["kind"],"C_action":action(r,"C"),
                           "C_fixed":correct(r,"C")} for r in a_wrong]},
           "hard_A_to_C":{"fixed":thC[1],"destroyed":thC[2]},
           "C_actions":{"keep":keep,"change_declared":change,"change_flipped":flip,
                "change_flipped_wrong":flip_bad,"change_declared_no_flip":declared_no_flip},
