...
rows=con.execute("select pid,romaji,ja,qid,mv,tm_url,dob_year,surname,stratum,nclub from pool where nclub>=2 order by pid").fetchall()
...
def take(cands,k):
    ...
    for p in cands:
        ...
        used.add(p["pid"]); out.append(p)
        if len(out)==k: break
    return out
# --- positives (unchanged) ---
conf_same=[]
for s,k in (("obscure",24),("mid",24),("famous",12)): conf_same+=take(by_str[s],k)
hard_pos=take([p for p in by_str["obscure"] if p["pid"] not in used],20)

# --- era-matched same-surname negatives ---
bysn=defaultdict(list)
for p in P:
    if p["pid"] not in used and p["dob_year"]: bysn[p["surname"]].append(p)
...
for gap,_,a,b in cand:
    if a["pid"] in used or b["pid"] in used: continue
    used.add(a["pid"]); used.add(b["pid"]); neg.append((a,b,gap))
    if len(neg)==80: break
hard_neg=neg[:20]; conf_diff=neg[20:80]

def mk(kind,truth,jp,ro):
    return dict(kind=kind,truth=truth,jp_pid=jp["pid"],jp_name=jp["ja"],jp_qid=jp["qid"],
                jp_url=f"https://www.wikidata.org/wiki/{jp['qid']}",ro_pid=ro["pid"],ro_name=ro["romaji"],
                ro_tm_url=ro["tm_url"],jp_stratum=jp["stratum"],ro_stratum=ro["stratum"])
pairs=[mk("CONF","SAME",p,p) for p in conf_same]+[mk("HARD","SAME",p,p) for p in hard_pos]
pairs+=[mk("CONF","DIFFERENT",a,b) for a,b,_ in conf_diff]+[mk("HARD_NEG","DIFFERENT",a,b) for a,b,_ in hard_neg]
for i,pr in enumerate(pairs): pr["pair_id"]=f"P{i+1:03d}"
...
with open(ROOT+"/data/phase1/gold_audit_sheet.csv","w",encoding="utf-8-sig",newline="") as f:
    w=csv.writer(f); w.writerow(["pair_id","kind","truth_by_construction","name_ja","jp_wikidata_url","name_latin","romaji_transfermarkt_url","human_verdict(SAME/DIFFERENT/UNSURE)","note"])
    for pr in pairs: w.writerow([pr["pair_id"],pr["kind"],pr["truth"],pr["jp_name"],pr["jp_url"],pr["ro_name"],pr["ro_tm_url"],"",""])

import statistics as st
gaps=[g for _,_,g in neg]
print(f"pairs={len(pairs)} | SAME={sum(p['truth']=='SAME' for p in pairs)} DIFFERENT={sum(p['truth']=='DIFFERENT' for p in pairs)}")
print(f"neg birth-year gap: mean={st.mean(gaps):.1f} median={st.median(gaps):.0f} max={max(gaps)} | <=2yr:{sum(1 for g in gaps if g<=2)}/80 | same-full-name:{sum(1 for a,b,_ in neg if a['romaji']==b['romaji'])}")
