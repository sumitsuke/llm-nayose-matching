...
con.execute(f"""create temp table pool as
  ...
  where l.ja <> ''""")
def clubs(pid,k=4):
    r=con.execute("""select c.name,cast(min(a.date) as varchar),cast(max(a.date) as varchar),count(*) n
...
step=max(1,len(pos)//10); pos_players=pos[::step][:10]
...
def rec(lbl,name,cl):
    return "\n".join([f"レコード{lbl}:",f"  氏名: {name}","  所属クラブ歴:"]+
...
def build(nameA,clA,nameB,clB,prior=None):
    ...
    if prior is not None:
        P+=["","初回判定（叩き台・正しいとは限らない）:",prior,"","上記を独立に再監査し KEEP か CHANGE を選べ。変更自体を目的にしないこと。"]
    return "\n".join(P)

pairs=[]
for pid,romaji,ja,mv in pos_players:
    cl=clubs(pid); pairs.append(("SAME", ja, cl, romaji, cl))
...
LEAK={"QID":r"\bQ\d+\b","id_kw":r"player_id|spieler|transfermarkt","url":r"https?://|\.r2\.dev|wikidata","digits>=5":r"\d{5,}"}
hits={k:0 for k in LEAK}; examples=[]
for _,nA,clA,nB,clB in pairs:
    t=build(nA,clA,nB,clB)
    for k,p in LEAK.items():
        if re.search(p,t): hits[k]+=1
    if len(examples)<5: examples.append(t)
json.dump({"leak_hits":hits,"example_prompts":examples}, open(ROOT+"/data/phase0/dev_smoke_prompts.json","w",encoding="utf-8"), ensure_ascii=False, indent=1)
...
def pj(s):
    try: return json.loads(s)
        ...
        m=re.search(r"\{.*\}",s,re.S); return json.loads(m.group()) if m else None
rows=[]
for truth,nA,clA,nB,clB in pairs:
    base=build(nA,clA,nB,clB)
    a=call(base); b=call(base); c=call(build(nA,clA,nB,clB,prior=a))
...
# ---- report (ASCII) ----
n=len(rows); dec=lambda x:(x or {}).get("final_decision")
parse_ok=sum(all(r[k] for k in "ABC") for r in rows)
A_acc=sum(dec(r["A"])==r["truth"] for r in rows); C_acc=sum(dec(r["C"])==r["truth"] for r in rows)
...
print(f"MODEL={MODEL} eff={EFFORT} | pairs={n} (POS={sum(r['truth']=='SAME' for r in rows)}, NEG={sum(r['truth']=='DIFFERENT' for r in rows)})")
print(f"JSON parse OK (A&B&C): {parse_ok}/{n}")
print(f"accuracy A={A_acc}/{n} C={C_acc}/{n}  [pos->SAME {pos_same}/{sum(r['truth']=='SAME' for r in rows)}, neg->DIFFERENT {neg_diff}/{sum(r['truth']=='DIFFERENT' for r in rows)}]")
