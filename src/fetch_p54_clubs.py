...
def wb(ids, props, extra=None):
    ...
    for a in range(4):
        try:
            return json.loads(urllib.request.urlopen(urllib.request.Request(url,headers=UA),timeout=60).read())["entities"]
        except Exception as e:
            print(f"  wb retry {a}: {type(e).__name__}",flush=True); time.sleep(5*(a+1))
    return {}
...
def yr(quals,pid):
    try: return quals.get(pid,[{}])[0]["datavalue"]["value"]["time"][1:5]
    except Exception: return ""
by_player={}; club_qids=set()
for batch in chunks(player_qids,50):
    ents=wb(batch,"claims")
    for q,e in ents.items():
        lst=[]
        for cl in e.get("claims",{}).get("P54",[]):
            try: cq=cl["mainsnak"]["datavalue"]["value"]["id"]
...
with open(ROOT+"/data/phase1/wd_clubs.csv","w",encoding="utf-8",newline="") as f:
    w=csv.writer(f); w.writerow(["qid","club","start","end"]); rows=0
    for q,lst in by_player.items():
        for cq,s,e in lst:
            w.writerow([q, labels.get(cq,cq), s, e]); rows+=1
