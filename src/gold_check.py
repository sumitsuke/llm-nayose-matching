...
UA = {"User-Agent": "llm-nayose/0.1 (research)"}
...
def wb(ids):
    url = (
        API
        + "?"
        + urllib.parse.urlencode({"action": "wbgetentities", "ids": "|".join(ids), "props": "claims", "format": "json"})
    )
    for a in range(4):
        try:
...
tm_dob = {
    ...
    for r in con.execute(
        "select player_id,date_of_birth from players where player_id in (" + ",".join(map(str, pids)) + ")"
    ).fetchall()
}
...
flag = []
for p in pairs:
    wd = wd_dob.get(p["jp_qid"], ("", 0))
    tmr = tm_dob.get(p["ro_pid"], "")
    m = match(wd, tmr)
    if p["truth"] == "SAME":
        ...
        gold = (
            "DIFFERENT(DOB相違)"
            if m is False
            else ("要人手(DOB欠)" if m is None else "!要確認:DIFFなのにDOB一致=重複疑い")
        )
        ok = m is False
    if m is None:
        nod += 1
        ...
        flag.append((p["pair_id"], p["kind"], p["truth"], p["jp_name"], p["ro_name"], wd[0], tmr))
    rows.append(
        dict(
            pair_id=p["pair_id"],
            kind=p["kind"],
            construction_label=p["truth"],
            name_ja=p["jp_name"],
            name_latin=p["ro_name"],
            wikidata_qid=p["jp_qid"],
            wd_dob=wd[0],
            wd_prec=wd[1],
            dcaribou_dob=tmr,
            mechanical_gold=gold,
        )
    )
...
for pid in ["P141", "P142"]:
    ...
    print(
        f"  {pid} {r['name_ja']}/{r['name_latin']} wd_dob={r['wd_dob']} dcaribou_dob={r['dcaribou_dob']} -> {r['mechanical_gold']}"
    )
