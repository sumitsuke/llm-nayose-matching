...
for kind, nameA, clA, nameB, clB in pairs:
    ...
    for cond, x in (("A", a), ("B", b), ("C", c)):
        log[cond].append(x)
        raw.append({"kind": kind, "cond": cond, **{k: x[k] for k in ("inp", "out", "reason")}, "content": x["content"]})
...
print(
    f"MODEL={MODEL} effort={EFFORT} | pairs={len(pairs)} (POS={sum(1 for p in pairs if p[0] == 'POS')}, NEG={sum(1 for p in pairs if p[0] == 'NEG')}) | calls={len(raw)}"
)
allin = allout = 0
for cond in ("A", "B", "C"):
    i = avg([x["inp"] for x in log[cond]])
    o = avg([x["out"] for x in log[cond]])
    rz = avg([x["reason"] for x in log[cond]])
    print(f"  {cond}: in~{i:.0f}  out~{o:.0f} (reasoning~{rz:.0f})  n={len(log[cond])}")
per_in = avg([x["inp"] for x in raw])
per_out = avg([x["out"] for x in raw])
print(f"  per-call avg: in~{per_in:.0f}  out~{per_out:.0f}")

# --- extrapolate to full run ---
CALLS = 620
tot_in = per_in * CALLS
tot_out = per_out * CALLS
print(f"\nFULL RUN extrapolation (@{CALLS} calls): input~{tot_in / 1e6:.3f}M  output~{tot_out / 1e6:.3f}M tokens")
PRICES = {
    "gpt-5.4-mini": (0.75, 4.50),
    "gpt-5.4-nano": (0.20, 1.25),
    "gpt-5.6-luna(proj)": (1.00, 6.00),
    "gpt-5.4": (2.50, 15.0),
}
