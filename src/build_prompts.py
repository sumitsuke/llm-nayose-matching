...
def tm_clubs(pid, k=6):
    return con.execute(
        """select to_club_name, min(transfer_season) s from transfers
      where player_id=? and to_club_name is not null group by to_club_name order by s limit ?""",
        [pid, k],
    ).fetchall()
...
for want in [("CONF", "SAME"), ("CONF", "DIFFERENT"), ("HARD_NEG", "DIFFERENT"), ("HARD", "SAME")]:
    for o, p in zip(out, pairs):
        if (p["kind"], p["truth"]) == want:
            ex.append(f"=== {o['pair_id']} {want} ===\n{o['prompt_A']}\n")
            break
