    ...
    for a in range(5):
        try:
            r = client.chat.completions.create(
                model=MODEL, messages=[{"role": "user", "content": text}], reasoning_effort=EFFORT
            )
...
for p in prompts:
    ...
    a = call(p["prompt_A"])
    b = call(p["prompt_A"])
    c = call(p["prompt_A"] + C_SUFFIX.format(a["content"]))
    for x in (a, b, c):
        cost += x["inp"] / 1e6 * IN_P + x["out"] / 1e6 * OUT_P
    results.append(
        dict(
            pair_id=p["pair_id"],
            kind=p["kind"],
            ...
            C_raw=c["content"],
            tokens={
                "A": {k: a[k] for k in ("inp", "out", "reasoning")},
