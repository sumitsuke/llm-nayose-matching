...
req = urllib.request.Request(
    url,
    headers={"User-Agent": "llm-nayose-feasibility/0.1 (research use)", "Accept": "application/sparql-results+json"},
)
for attempt in range(1, 6):
    try:
        ...
        print(f"attempt {attempt}: HTTP {e.code}; sleeping {wait}s", flush=True)
        time.sleep(wait)
    except Exception as e:
        print(f"attempt {attempt}: {type(e).__name__} {e}; sleeping 20s", flush=True)
        time.sleep(20)
